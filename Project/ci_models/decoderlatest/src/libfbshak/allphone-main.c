...
typedef struct t_arg {
    ...
    senone_t *sen;
    float32 ***feat;
    gauden_dist_t **dist;        /* Density values for one mgau in one frame */
    int32 **senscr;              /* Senone scores for window of frames */
    mgau2sen_t **mgau2sen;
} t_args;


void *Compute_senscr(void *threadarg)
{
    int32 i, j, k, gid, s;
    int32 firstgau, lastgau, nfr, topn, thrid;

    gauden_t *gden;		/* Gaussian density codebooks */
    float32 ***feat;
    gauden_dist_t **dist;        /* Density values for one mgau in one frame */
    int32 **senscr;              /* Senone scores for window of frames */
    ...
    t_args *args = (t_args *) threadarg;

    thrid = args->thrid;
    gden = args->g;
    firstgau = args->firstgau;
    lastgau = args->lastgau;
    nfr = args->nfr;
    topn = args->topn;
    feat = args->feat;
    dist = args->dist;
    senscr = args->senscr;
    mgau2sen = args->mgau2sen;
    gsen = args->sen;

    for (gid = firstgau; gid < lastgau; gid++) {
        for (i = j, k = 0; (k < GAUDEN_EVAL_WINDOW) && (i < nfr); i++, k++) {
            /* Evaluate mixture Gaussian densities */
            gauden_dist (gden, gid, topn, feat[k], dist);

            /* Compute senone scores */
            if (gden->n_mgau > 1) {
                for (m2s = mgau2sen[gid]; m2s; m2s = m2s->next) {
                    s = m2s->sen;
                  senscr[k][s] = senone_eval (gsen, s, dist, topn);
                }
            } else {
                /* Semi-continuous special case; single shared codebook */
                senone_eval_all (gsen, dist, topn, senscr[k]);
            }
        }
    }
    return;
...
static void write_phseg (char *ctlspec, char *dir, char *uttid, phseg_t *phseg)
{
    char str[1024], buf[1024];
    FILE *fp=NULL;
    int32 uttscr, f, scale, k, writephsegtolog;
    
    /* Attempt to write segmentation for this utt to a separate file */

    if (dir) {
        k = strlen(dir);
        if ((k > 4) && (strcmp (dir+k-4, ",CTL") == 0)) { 
            if (ctlspec[0] != '/') {
                strcpy (buf, dir);
                buf[k-4] = '/';
    ...
    writephsegtolog = dir || outsegfp ? 0 : 1;

    if (dir) {
	sprintf (str, "%s.allp", buf, uttid);
    ...
    if (dir || writephsegtolog)
        fprintf (fp, "\t%5s %5s %9s %s\n",
    	     "SFrm", "EFrm", "SegAScr", "Phone");
    
    uttscr = 0;
    if (outsegfp) 
        fprintf(outsegfp,"%s SYMBOL ENDFRM (SCORE) 0 ", uttid);
	...
	if (writephsegtolog)
        ...
        if (outsegfp)
            fprintf(outsegfp,"%s %d (%d) ", mdef_ciphone_str(mdef, phseg->ci),
                                            phseg->ef, phseg->score+scale);
 ...
 */
static void allphone_utt (float32 **mfc, int32 nfr, char *ctlspec, char *uttid)
{
    ...
    static int    ithr, startgau, count, rc, threadsinit=0;
    static pthread_t thread[NUM_THREADS];
    static t_args *thrargs = NULL;
    static pthread_attr_t attr;
    static void *status;

    if (!thrargs)
        thrargs = (t_args*) calloc(NUM_THREADS, sizeof(t_args));

    if (! feat) {
	/* One-time allocation of necessary intermediate variables */

	/* Allocate space for a feature vector */
	feat = (float32 ***) ckd_calloc_2d (GAUDEN_EVAL_WINDOW, 
                                         n_feat, sizeof(float32 *));
	for (j = 0; j < GAUDEN_EVAL_WINDOW; j++) {
	    for (i = 0; i < n_feat; i++) {
                k = featlen[i];
                /* Align to 4-float boundary for fast Gaussian scoring */
                if (k%4 != 0) k += (4 - k%4);
	        feat[j][i] = (float32 *) ckd_calloc (k, sizeof(float32));
            }
        }
    ...
    if (!threadsinit){
        for (ithr=0; ithr<NUM_THREADS; ithr++) {
            thrargs[ithr].thrid = ithr;
            thrargs[ithr].topn = topn;
            thrargs[ithr].g = g;        
            thrargs[ithr].feat = feat;
            thrargs[ithr].dist = dist;   
            thrargs[ithr].senscr = senscr;  
            thrargs[ithr].sen = sen;  
            thrargs[ithr].mgau2sen = mgau2sen;
            rc = pthread_create(&thread[ithr],NULL, Compute_senscr, 
                               (void*)&thrargs[ithr]);
            if (rc)
                E_FATAL("Return err code from pthread_create is %d\n",rc);
        }
        threadsinit = 1;
    }
	...
	for (i = j, k = 0; (k < GAUDEN_EVAL_WINDOW) && (i < nfr); i++, k++) {
	    ...
	    if (i < w)
	        feat_cep2feat (mfc+w, feat[k]);
	    else if (i >= nfr-w)
	        feat_cep2feat (mfc+(nfr-w-1), feat[k]);
	    else
	        feat_cep2feat (mfc+i, feat[k]);
        }
        for (ithr=0, i = 0; ithr<NUM_THREADS; ithr++) {
            thrargs[ithr].nfr = nfr;
            thrargs[ithr].firstgau = i;
            j = i + g->n_mgau/NUM_THREADS;
            if (ithr == NUM_THREADS) j = g->n_mgau;
            thrargs[ithr].lastgau = j;
            i = j;
            rc = pthread_create(&thread[ithr],NULL, Compute_senscr, 
                               (void*)&thrargs[ithr]);
            if (rc) E_FATAL("Return err code from pthread_create is %d\n",rc);
        }
        for (ithr=0; ithr<NUM_THREADS; ithr++) {
            rc = pthread_join(thread[ithr], &status);
            if (rc) E_FATAL("return code from pthread_join() is %d\n",rc);
        }
    ...
    write_phseg (ctlspec, phsegdir, uttid, phseg);
...
{
    FILE *ctlfp;
    char *ctlfile, *cepdir, *cepext, *outsegfn, *arg;
...
/* END OF CHANGES BY BHIKSHA */
    ...
    outsegfn = NULL;
    if ((arg = (char *) cmd_ln_access ("-outsegfn")) != NULL)
        outsegfn = (char *) ckd_salloc (arg);

    if (outsegfn) {
        E_INFO("Writing segmentation output to %s\n",outsegfn);
        if ((outsegfp = fopen (outsegfn, "w")) == NULL)
    	    E_FATAL("Unable to open output file %s for writing\n", outsegfn);
    }
    else  
        outsegfp = NULL;
...
/* END CHANGES BY BHIKSHA */
	    ...
	    E_INFO ("%d mfc frames\n", nfr);
	    allphone_utt (mfc, nfr, ctlspec, uttid);
	}
