...
/* END CHANGES BY BHIKSHA */
    ...
    { "-skipframe",
      ...
      CMD_LN_NO_VALIDATION,
      "1",
      "Every skipframe-th frame will be skipped during decoding" },
    { "-topn",
      CMD_LN_INT32,
      CMD_LN_NO_VALIDATION,
      "4",
...
{
    static float32 ***featblk = NULL;	/* Input feature vector in current frame */
    static int32 w;
    static int32 topn;
	...
	/* One-time allocation of necessary intermediate variables */
        ...
        for (j = 0; j < GAUDEN_EVAL_WINDOW; j++) {
            for (i = 0; i < n_feat; i++) {
                ...
                if (k%4 != 0) k += (4 - k%4);
                featblk[j][i] = (float32 *) ckd_calloc (k, sizeof(float32));
            }
        }
     ...
     */
    if (sen_active) {
        float32 **featfrm = featblk[0]; /* Only need one frame */
	for (i = 0; i < nfr; i++) {
            if (skipframe > 1 && i%skipframe == 0) continue;
	    timing_start (tm_gausen);

	    /* Compute feature vector for current frame from input speech cepstra */
	    if (i < w)
		feat_cep2feat (mfc+w, featfrm);
	    else if (i >= nfr-w)
		feat_cep2feat (mfc+(nfr-w-1), featfrm);
	    else
		feat_cep2feat (mfc+i, featfrm);
	    ...
	    for (gid = 0; gid < g->n_mgau; gid++) {
		if (mgau_active[gid]) {
		    gauden_dist (g, gid, topn, featfrm, dist);
	...
	for (j = 0; j < nfr; j += GAUDEN_EVAL_WINDOW) {
	    ...
	    for (i = j, k = 0; (k < GAUDEN_EVAL_WINDOW) && (i < nfr); i++, k++) {
                if (skipframe > 1 && i%skipframe == 0) continue;
	        /* Compute feature vector for current frame from input cepstra */
	        if (i < w)
		    feat_cep2feat (mfc+w, featblk[k]);
	        else if (i >= nfr-w) 
		    feat_cep2feat (mfc+(nfr-w-1), featblk[k]);
	        else
		    feat_cep2feat (mfc+i, featblk[k]);
            }
	    for (gid = 0; gid < g->n_mgau; gid++) {
		for (i = j, k = 0; (k < GAUDEN_EVAL_WINDOW) && (i < nfr); i++, k++) {
                    if (skipframe > 1 && i%skipframe == 0) continue;
		    ...
		    gauden_dist (g, gid, topn, featblk[k], dist);
...
static void decode_utt (float32 **mfc, int32 nfr, char *uttid)
{
    char *bscrdir;
    hyp_t *hyp, *h;
    int32 i, bp, ascr, lscr, scl, skipframe;
    float32 *f32arg;
    float64 lwf;
