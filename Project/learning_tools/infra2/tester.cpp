...
int test_vector()
{
  ...
  // text files
  ofstream os("tmp_file.txt");
  os << v2 << "\n" << v3 << "\n";
  os.close();
  ifstream is("tmp_file.txt");
  is >> v4 >> v5;
  check_bug(!(v4 != v2 || v5 != v3),"text IO");
  is.close();

  ifstream is2("tmp_file.txt");
...
int test_vector_base()
{
  ...
  FILE *fin = fopen("tmp_file.infra","rb");
  s = 0;
