...
static int32 beam;
static int32 inspen;
static int32 mdef_has_triphones;
static int32 fastdecode_cionly_nophonetp;
...
static int32 phmm_link ( void )
{
	...
	for (p = ci_phmm[ci]; p; p = p->next) {
	    /* Build rclist for p */
	    i = 0;
            if (!fastdecode_cionly_nophonetp) {
...
#if 0 // RSINGH commented this out on 25 april 2014.
      // The first phone is not required to be SIL anymore
...
#else // And replaced it with this
int32 allphone_start_utt (char *uttid)
{
    s3cipid_t ci;
    phmm_t *p;
    ...
    /* Initialize start state of all phoneme HMMs */
    curfrm = 0;
    for (ci = 0; ci < mdef->n_ciphone; ci++) {
	for (p = ci_phmm[ci]; p; p = p->next) {
	    p->active = curfrm;
...
static void phmm_trans ( void )
{
    ...
    /* Do the full transition if tp is provided */
    if (tp != NULL && !fastdecode_cionly_nophonetp) {
    ...
    /* Also need full transition evaluation if we have triphones */
    else if (mdef_has_triphones && !fastdecode_cionly_nophonetp) {
        for (h = frm_hist[curfrm]; h; h = h->next) {
	    from = h->phmm;
	    newscore = h->score + inspen;
            if (newscore > beam) {
	        for (l = from->succlist; l; l = l->next) {
	            to = l->phmm;
	            if (newscore > to->inscore) {
		        to->inscore = newscore;
		        to->inhist = h;
		        to->active = nf;
	            }
    ...
    /* Else we need a much simpler CI based transition */
    else {
        int32 bestscore, hscore;
        history_t *besth;
        ...
        bestscore += inspen;
        if (bestscore > beam) {
            /* Since we're working with CI phones, all successor lists are idential
             * We an follow any one. We'll just follow the one from besth
             */
	    from = besth->phmm;
	    for (l = from->succlist; l; l = l->next) {
	        to = l->phmm;
	        if (bestscore > to->inscore) {
		    to->inscore = bestscore;
		    to->inhist = besth;
		    to->active = nf;
	        }
	    }
...
int32 allphone_init ( void )
{
    ...
    cionlyflag = (char *) cmd_ln_access ("-cisuperfast");
    if (strcmp(cionlyflag,"yes") != 0 && strcmp(cionlyflag,"no") != 0)
        E_ERROR("-cisuperfast flag must be set to yes or no; assuming no\n");
    fastdecode_cionly_nophonetp = 0;
    if (strcmp(cionlyflag,"yes")==0) fastdecode_cionly_nophonetp = 1;
    if (!fastdecode_cionly_nophonetp) {
    ...
    mdef_has_triphones = (mdef->n_ciphone == mdef->n_phone) ? 0 : 1;
