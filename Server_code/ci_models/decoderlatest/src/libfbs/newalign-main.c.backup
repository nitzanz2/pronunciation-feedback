...
/* END CHANGES BY BHIKSHA */
    ...
    { "-outctlfn",
      ...
      CMD_LN_NO_VALIDATION,
      CMD_LN_NO_DEFAULT,
      "Output control file with list of succesfully aligned files" },
    { "-stsegdir",
      CMD_LN_STRING,
      CMD_LN_NO_VALIDATION,
      CMD_LN_NO_DEFAULT,
...
/* Write out control file entry */
static void write_outctl (FILE *fp, char *ctlline)
{
    fprintf(fp,"%s",ctlline);
    fflush(fp);
}
...
static void align_utt (char *sent,	/* In: Reference transcript */
		       ...
		       char *ctlspec,	/* In: Utt specifiction from control file */
                       ...
                       char *ctlline)      /* The entire control line */

{
    ...
    int32 i, k, s, sid, gid, n_sen_active, best;
    char *arg;
    align_stseg_t *stseg;
    align_phseg_t *phseg;
    align_wdseg_t *wdseg;
	...
	/* One-time allocation of necessary intermediate variables */
        ...
        for (i = 0; i < n_feat; i++) {
            ...
            if (k%4 != 0) k += (4 - k%4);
            feat[i] = (float32 *) ckd_calloc (k, sizeof(float32));
        }
	...
	if (outsentfp)
        ...
        if (outctlfp)
            write_outctl (outctlfp, ctlline);
    }
/* HACK BY BHIKSHA: ADDED TAG */
cleanup:
...
static void process_ctlfile ( void )
{
    ...
    char *ctlfile, *cepdir, *cepext, *sentfile;
    char *outsentfile, *outctlfile, *mllrctlfile;
...
/* END OF CHANGES BY BHIKSHA */
    ...
    if ((outctlfile = (char *) cmd_ln_access("-outctlfn")) != NULL) {
...
/* END CHANGES, 6 JAN 1998, BHIKSHA */
	...
	if ((k > 0) && (sent[k] == ')')) {
            /* for (--k; (k >= 0) && (sent[k] != '('); --k); */
            /* Above replaced by following two lines: BHIKSHA  */
            for (--k; (k >= 0) && (sent[k] != ' ' && sent[k] != '\t'); --k);
            /* replaced sent[k] == '(' with the term in the parantheses: BHIKSHA */
            if ((k >= 0) && (sent[k] == ' ' || sent[k] == '\t')) {
		sent[k] = '\0';
...
/* CHANGE BY BHIKSHA; PASSING VECLEN TO s2mfc_read(), 6 JAN 98 */
	/* Read mfc file */
	/* HACK HACKA HACK BHIKSHA: FUZZIFY sf AND ef BY 4 */
	if (sf < 4 ) {
	    E_ERROR("Utterance %s begin %d < 4; ignored\n", uttid, sf);
	    return;
	}
	sf -= 4; /* Account for boundary needed to compute deltas */
	ef += 4; /* Account for boundary needed to compute deltas */
...
/* END CHANGES BY BHIKSHA */
	    ...
	    /* Align utterance */
	    align_utt (sent, mfc+4, nfr-8, ctlspec, uttid, line); /* +4 HACKA HACK */
	}
